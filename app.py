...
if agree or disagree:
    ...
    if st.session_state.submitted:
        ...
        st.session_state.how = "might be **a bit " if np.linalg.norm(st.session_state.dn) < 3 else "might be **very " if np.linalg.norm(st.session_state.dn) < 8 else "might be **overly "
